...
int main(void) {
    ...
    SDL_Surface *tex_surface = IMG_Load("D:\\media\\xgu.png");
    ...
    uint8_t *alloc_tex_dsdt = MmAllocateContiguousMemoryEx(tex_dsdt_pitch * tex_dsdt_height, 0, 0x03FFAFFF, 0, PAGE_WRITECOMBINE | PAGE_READWRITE);
    ...
    float bumpenv_mat[2*2] = {
        32.0f/(float)tex_surface->w, 0.0,
        0.0, 32.0f/(float)tex_surface->h,
    };
    
    p = xgu_set_texture_set_bump_env_mat(p, 1, bumpenv_mat);
    p = xgu_set_texture_set_bump_env_scale(p, 1, 1.0f);
    p = xgu_set_texture_set_bump_env_offset(p, 1, 0.0f);
